...
void main()
{
	  ...
	  s3=s1+s2;        //隐式调用operator+()，可解释为operator+(s1,s2)（显示调用）；
	  cout<<"s3=s1+s2=";
	  s3.print();    
	  s4=s1-s2;        //隐式调用operator-()，可解释为operator-(s1,s2)（显示调用）；
	  cout<<"s4=s1-s2=";
	  s4.print();
	  s5=s1*s2;       //隐式调用operator*()，可解释为operator*(s1,s2)（显示调用）;
	  cout<<"s5=s1*s2=";
	  s5.print();
	  s6=s1/s2;        //隐式调用operator/()，可解释为operator/(s1,s2)（显示调用）；
	  cout<<"s6=s1/s2=";
	  s6.print();
	  s7=(s1+s3)*s2-(s4-s5)/s6;              //重载运算符的混合运算；
